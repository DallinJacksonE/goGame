...
class GoGame:
    def __init__(self, size=9):
        ...
        self.pastBoard = copy.deepcopy(self.board)
        self.currentTurn = 'black'
        self.capturedStones = {'black': 0, 'white': 0}

    def getOpponent(self, color):
        print("GetOpponent: ", color)
    ...
    def placeStone(self, x, y):
        ...
        # Make temporary board to edit
        tempBoard = copy.deepcopy(self.board)
        tempBoard[y][x] = self.currentTurn
        opponent = self.getOpponent(self.currentTurn)
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if self.onBoard(nx, ny):
                if tempBoard[ny][nx] == opponent:
                    group, liberties = self.getGroup(nx, ny, tempBoard)
                    if liberties == 0:
        ...
        for group in captured_groups:
            for cx, cy in group:
                tempBoard[cy][cx] = None
                stones_captured_count += 1

        # Check for Suicide
        my_group, my_liberties = self.getGroup(x, y, tempBoard)
        if my_liberties == 0 and stones_captured_count == 0:
            return False, "Suicide move is not allowed"

        # Move leaves board in same state (KO)
        if tempBoard == self.pastBoard:
            return False, "KO"
        ...
        # Add to score
        self.capturedStones[self.currentTurn] += stones_captured_count
